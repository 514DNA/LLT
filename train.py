...
from GAVE.nnutils.pcreg_trainer import PCReg_Trainer
...
if __name__ == "__main__":
    ...
    config_path = os.path.join("GAVE/configs", args.config_path)
    cfg.merge_from_file(config_path)

    # deal with modifiers
    if args.modifier is not None:
        assert cfg.EXPERIMENT.name == "", "Modifier config defines experiment name."
        modifier_path = os.path.join("GAVE/configs", args.modifier)
        cfg.merge_from_file(modifier_path)
   # else:
   #     modifier_path = "GAVE/configs/config.py"
    #    cfg.merge_from_file(modifier_path)
