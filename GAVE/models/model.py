...
from .backbones import ResNetDecoder, ResNetEncoder,DepthEncoder,DCNEncoder
...
class PCReg(nn.Module):
    def __init__(self, cfg):
        ...
        pretrained = False
        self.encode = DCNEncoder(chan_in, feat_dim, pretrained)
        self.decode = ResNetDecoder(feat_dim, 3, nn.Tanh(), pretrained)
        self.depthencoder = DepthEncoder(pretrained)
    ...
    def forward(self, rgbs, K, deps, vps=None,istest=False):
        # Estimate Depth -- now for 1 and 2
        n_views = len(rgbs)
        output = {}

        # Encode features
        ds = [self.depthencoder(deps[i],rgbs[i]) for i in range(n_views)]
       # feats = [torch.cat([rgbs[i],ds[i]],axis = 1) for i in range(n_views)]
        feats = [self.encode(rgbs[i],ds[i]) for i in range(n_views)]
...
#        if vps is not None:
 ...
 #       elif self.align_cfg.algorithm == "weighted_procrustes":
        ...
        for i in range(1, n_views):
            ...
            if gts is None:
                Rt_i, cor_loss_i = align(corr_i, pcs_X[0], pcs_X[i], None ,self.align_cfg)
            else:
                Rt_i, cor_loss_i= align(corr_i, pcs_X[0], pcs_X[i], gts[i - 1] ,self.align_cfg)
...
#            raise ValueError(f"How to align using {self.align_cfg.algorithm}?")
        ...
        output["joint_pointcloud"] = torch.cat(pcs_X, dim=1)
        ...
        if gts is None:
            output["render0"] = projs[0]["feats"][:, -3:]
            output["render1"] = projs[1]["feats"][:, -3:]
            output["rgb0"] = rgbs[0]
            output["rgb1"] = rgbs[1]
            output["pointclouds"] = pcs_X
            output["pointclouds_rgb"] = pcs_rgb
            return output
