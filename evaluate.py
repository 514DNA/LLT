...
from GAVE.configs import get_cfg_defaults
from GAVE.datasets import build_loader
from GAVE.models import build_model
from GAVE.models.model_util import get_grid
from GAVE.utils.metrics import evaluate_correspondances, evaluate_pose_Rt
...
def evaluate_split(model, data_loader, args, dict_name=None, use_tqdm=True):
    ...
    #err = torch.cat([r_err.unsqueeze(0),t_err.unsqueeze(0)],axis = 0)
...
def forward_batch(model, batch):
    ...
    output = model(gt_rgb, K=K, deps=gt_dep,vps = gt_vps)
    ...
    if isinstance(model, torch.nn.DataParallel):
        gt_pc = model.module.generate_pointclouds(K, gt_dep, gt_vps)
    else:
